...
**任务**：这个项目要求长期更新，长期优化。基础知识每个做6道题后，开始将高级知识也加入做题的队列中。注意每一个单元不只是有题，
还有关于这个数据结构或者是算法的必备知识，这些知识都是要掌握的重点也是面试的重点甚至可以说是走向高级开发者必经的道路。也需要去解决掉。配套的知识点的问题按照题一样考虑。
这样不光是做题，配套的知识点也可以解决掉了。 这个项目不是为了把leetcode全部做完，为的是把每个单元都搞懂，只要把每个类型的不同题型都做完了都懂了，就OK了，这个项目不是为了把leetcode全部做了
